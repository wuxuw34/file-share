...
function sendFileList() {
    ...
    (fileList.value as File[]).forEach((file: File, index: number) => {
        handleSendSingleFile(file, (r: MessageResultType | null) => {
            if (r) {
                const { state, data, msg } = r
                requestCode()
                if (state) {
                    fileStatus.value[index] = 1 // 修改文件状态
                } else {
                    fileStatus.value[index] = -1 // 修改文件状态
                }
            } else {
                fileStatus.value[index] = -1
            }
        })
    })
}
...
async function handleSendSingleFile(file: File, callback?: (r: MessageResultType | null) => void) {
...
function handleClickDownload(e: MouseEvent, index: number) {
    const a = document.createElement('a')
    a.download = 'true'
    a.href = window.$baseURL + 'upload/' + fileList.value[index].name
    a.click()
}
...
<template>
    <div>
        ...
        <el-button type="primary" style="margin-top: 5px;" @click="sendFileList">发送</el-button>
