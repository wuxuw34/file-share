...
onMounted(() => {
    ...
    window.$baseURL = 'http://47.108.159.217:8083/'
    handleResultURL()
        ...
        .then((r: MessageResultType) => {
            ...
            if (state) {
                switch (type) {
                    ...
                    case 'video': {
                        videoRef.value.requestAllScreen(JSON.parse(data.videos))
                        break
                    }
                    case 'file': {
...
function queryTypeByCode() {
    const url = window.$baseURL + 'getType?code=' + codeVal.value
    fetch(url, {
        method: 'get'
    }).then(r => r.json())
        .then(r => {
    ...
    <div class="home">
        ...
        <main style="width: var(--width);">
            ...
            <div class="operate_area">
                <el-tabs v-model="labelName" @tab-click="handleTabClick">
